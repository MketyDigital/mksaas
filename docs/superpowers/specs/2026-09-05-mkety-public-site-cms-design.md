...
## Addendum: App Experience, Platform Control Center, Auth Gateway, and Domains

The original specification above defines the public website, global docs, and public CMS. Later implementation work extended the same branch with app-experience and platform-control foundations. These extensions do not replace the original spec; they clarify adjacent admin-managed presentation surfaces.

- Public content tables manage `mkety.com` public website and `/docs` content.
- App-experience tables manage `app.mkety.com` dashboard, workspace-card, onboarding, and Platform Control Center presentation.
- The Platform Control Center is a dashboard of guarded modules, not an unrestricted system editor.
- The Auth Gateway is documented and represented as a protected module, but production signing/JWKS/token issuance remains out of this public CMS batch.
- Domains must follow the approved map: `mkety.com`, `app.mkety.com`, `api.mkety.com`, `origin.mkety.com`, and `*.mkety.app`.
- Billing, entitlements, deployment engines, tenant isolation, private keys, and security rules remain code-controlled with safe admin-facing operations only.
