...
import { AutomationWorkflowManualRunForm } from '@/features/projects/workspaces/automation/AutomationWorkflowManualRunForm';
...
import { AutomationWorkflowWebhookPanel } from '@/features/projects/workspaces/automation/AutomationWorkflowWebhookPanel';
...
export default async function AutomationBuilderPage({ params }: { params: Promise<{ tenant: string; project: string; workflow: string }> }) {
  ...
  return <WorkspaceShell projectName={access.project.name} projectSlug={access.project.slug} tenantSlug={access.tenant.slug} workspace={getProjectWorkspaceByKey('automation')}>
    {snapshot ? <div className="space-y-6">
      ...
      <AutomationWorkflowManualRunForm canManage={access.canManage} dependencyReadiness={snapshot.dependencyReadiness} preflight={snapshot.preflight} runtimeReadiness={snapshot.runtimeReadiness} projectSlug={access.project.slug} tenantSlug={access.tenant.slug} workflowSlug={snapshot.workflow.slug} />
      <AutomationWorkflowWebhookPanel appUrl={appUrl} canManage={access.canManage} endpoint={snapshot.webhookEndpoint} projectSlug={access.project.slug} tenantSlug={access.tenant.slug} workflowSlug={snapshot.workflow.slug} workflowTriggerType={snapshot.workflow.triggerType} />
    ...
    </div> : <WorkspaceEmptyState actions={[{ href: `/t/${access.tenant.slug}/projects/${access.project.slug}/automation`, label: 'Back to Automation Workspace' }]} description="This workflow record could not be found inside the current tenant and project." title="Workflow not found" />}
  </WorkspaceShell>;
}
