...
describe('POST /api/payments/flutterwave/webhook', () => {
  ...
  it('settles a verified SaaS payment locally', async () => {
  ...
  it('attests a verified Media event before forwarding the unchanged body', async () => {
    const rawBody = JSON.stringify({
      event: 'charge.completed',
      data: {
        id: 991,
  ...
  it('acknowledges valid non-charge Standard events without settlement', async () => {
    const rawBody = JSON.stringify({ event: 'transfer.completed', data: { id: 123 } });
  ...
  it('rejects missing or invalid Standard webhook hashes', async () => {
    const response = await POST(
      new Request('https://mkety.com/api/payments/flutterwave/webhook', {
        method: 'POST',
