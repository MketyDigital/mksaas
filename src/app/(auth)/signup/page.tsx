...
export default async function SignupPage({ searchParams }: SignupPageProps) {
  ...
  const authStart = new URL('/api/auth/login', 'https://mkety.com');
  authStart.searchParams.set('returnTo', selectTenantUrl);
  authStart.searchParams.set('intent', 'signup');
  redirect(`${authStart.pathname}${authStart.search}`);
}
