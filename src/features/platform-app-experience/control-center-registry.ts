...
export const platformControlModules: PlatformControlModule[] = [
  ...
  },
  {
    ...
    status: 'foundation',
    domain: 'multi-domain',
    editableScope: ['non-production deployment approvals', 'deployment history visibility', 'review notes'],
    protectedScope: ['deployment engine code', 'provider execution from approvals', 'production deployment approval', 'Cloudflare secrets', 'OCI secrets', 'DNS/custom-domain mutation', 'origin routing internals'],
    implementationNotes: 'Non-production request/approval records are implemented. Approval remains audit-only until a trusted project artifact pipeline and explicit execution handoff are verified.',
  },
  {
    key: 'domains-routing',
