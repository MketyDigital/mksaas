...
export async function runAgent(agent: AgentRuntimeDefinition, messages: ModelMessage[]) {
  ...
  return streamText({ model, system: buildSystemPrompt(agent, knowledgeContext), messages: normalizedMessages, tools, temperature: config.temperature, maxOutputTokens: config.maxOutputTokens, maxSteps: config.maxSteps });
}

export async function testAgent(agent: AgentRuntimeDefinition, messages: ModelMessage[]) {
  const { config, model, normalizedMessages, tools, knowledgeContext } = await prepare(agent, messages);
  return generateText({ model, system: buildSystemPrompt(agent, knowledgeContext), messages: normalizedMessages, tools, temperature: config.temperature, maxOutputTokens: config.maxOutputTokens, maxSteps: config.maxSteps });
}
