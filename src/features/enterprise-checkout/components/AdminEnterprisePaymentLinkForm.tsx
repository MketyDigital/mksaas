  ...
  return (
    <form onSubmit={submit} className="space-y-6 rounded-2xl border bg-card p-6 shadow-sm">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm font-medium">
          Enterprise scope
          <select className={`${fieldClass} mt-2`} name="scopeId" defaultValue="trading">
            <option value="trading">Trading Workspace / Infrastructure</option>
            ...
            <option value="infrastructure">Infrastructure & Deployment</option>
            <option value="enterprise">Other Enterprise Project</option>
          </select>
        </label>
      </div>
      ...
      <label className="block text-sm font-medium">
        ...
        <input className={`${fieldClass} mt-2`} name="projectName" required maxLength={180} />
      </label>
      <label className="block text-sm font-medium">
        Payment stage
        <input
          ...
          maxLength={160}
          placeholder="e.g. 50% deposit, milestone 2, final balance"
        />
      </label>
      <label className="block text-sm font-medium">
        Internal/customer-facing scope note
        <textarea className={`${fieldClass} mt-2 min-h-28`} name="projectDescription" maxLength={5000} />
      </label>
      <label className="block text-sm font-medium">
        Exact amount to collect (USD)
      ...
      {result ? (
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
          ...
          <button type="button" onClick={copyLink} className="mt-3 rounded-lg border px-3 py-2 text-sm font-medium">
            Copy payment link
          </button>
        </div>
      ) : null}
      ...
      <div className="rounded-xl bg-muted/50 p-4 text-xs leading-5 text-muted-foreground">
        Creating or paying this link does not automatically grant a subscription, wallet balance, credits,
        infrastructure, or workspace entitlement. Payment must be verified and fulfillment/access remains a separate
        controlled step.
      </div>
