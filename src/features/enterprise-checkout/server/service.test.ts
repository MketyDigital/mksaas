...
describe('enterprise checkout service', () => {
  it('creates the auditable order before invoking the provider', async () => {
    ...
    await service.createEnterpriseCheckout(request, { idempotencyKey: 'idempotency-1' });
  ...
  it('replays the same idempotent checkout without calling the provider again', async () => {
    ...
    const repository = {
      async findByIdempotencyKey() {
        return {
          ...
          providerPaymentReference: null, idempotencyKey: 'idempotency-1', metadata: { requestFingerprint: expect.any(String), redirectUrl: 'https://invoice.example/1' },
    ...
    const result = await service.createEnterpriseCheckout(request, { idempotencyKey: 'idempotency-1' });
  ...
  it('keeps the order auditable when the provider fails', async () => {
    ...
    await expect(service.createEnterpriseCheckout(request, { idempotencyKey: 'idempotency-2' })).rejects.toThrow('Enterprise checkout is temporarily unavailable.');
