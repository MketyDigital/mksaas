...
export function AutomationWorkflowManualRunForm({
  ...
  if (!canManage) {
    return (
      <section className="rounded-2xl border border-dashed bg-card p-5 md:p-6">
        <h3 className="text-base font-semibold">Manual execution is protected</h3>
        <p className="mt-2 text-sm text-muted-foreground">Only managers can start manual workflow runs.</p>
      </section>
    );
  }
  ...
  return (
    <section className="rounded-2xl border bg-card p-5 md:p-6">
      ...
      <h3 className="mt-1 text-base font-semibold">Internal manual execution</h3>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
        Runs trigger, transform, and condition nodes inside the auditable workflow run lifecycle. HTTP, Agent, webhooks, retries, credentials, and other external actions remain disabled.
      </p>
      ...
      {!ready ? <p className="mt-3 text-sm text-muted-foreground">Resolve every preflight error and warning before starting a manual run.</p> : null}
    </section>
  );
}
