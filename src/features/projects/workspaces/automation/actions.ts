...
export async function addAutomationWorkflowDraftNode(formData: FormData) { const nodeType = String(formData.get('nodeType') || ''); const { access, workflow } = await getManageableWorkflow(formData, 'You do not have permission to edit automation workflow nodes.'); const definition = buildWorkflowDefinitionWithDraftNode({ currentDefinition: workflow.definition, nodeType }); await db.update(workflows).set({ definition, updatedAt: new Date() }).where(and(eq(workflows.tenantId, access.tenant.id), eq(workflows.projectId, access.project.id), eq(workflows.id, workflow.id))); redirect(`/t/${access.tenant.slug}/projects/${access.project.slug}/automation/${workflow.slug}`); }
export async function updateAutomationWorkflowNodeConfigDraft(formData: FormData) { const nodeId = String(formData.get('nodeId') || ''); const { access, workflow } = await getManageableWorkflow(formData, 'You do not have permission to edit automation workflow node configuration.'); const definition = buildWorkflowDefinitionWithNodeConfigDraft({ currentDefinition: workflow.definition, nodeId, label: String(formData.get('label') || ''), notes: String(formData.get('notes') || ''), triggerMode: String(formData.get('triggerMode') || ''), agentId: String(formData.get('agentId') || ''), prompt: String(formData.get('prompt') || ''), method: String(formData.get('method') || ''), url: String(formData.get('url') || ''), headers: String(formData.get('headers') || ''), body: String(formData.get('body') || ''), input: String(formData.get('input') || ''), mapping: String(formData.get('mapping') || ''), field: String(formData.get('field') || ''), operator: String(formData.get('operator') || ''), value: String(formData.get('value') || '') }); await db.update(workflows).set({ definition, updatedAt: new Date() }).where(and(eq(workflows.tenantId, access.tenant.id), eq(workflows.projectId, access.project.id), eq(workflows.id, workflow.id))); redirect(`/t/${access.tenant.slug}/projects/${access.project.slug}/automation/${workflow.slug}`); }
export async function updateAutomationWorkflowNodeStructureDraft(formData: FormData) { const nodeId = String(formData.get('nodeId') || ''); const operation = String(formData.get('operation') || '') as WorkflowNodeStructureDraftOperation; const { access, workflow } = await getManageableWorkflow(formData, 'You do not have permission to edit automation workflow structure.'); const definition = buildWorkflowDefinitionWithNodeStructureDraft({ currentDefinition: workflow.definition, nodeId, operation }); await db.update(workflows).set({ definition, updatedAt: new Date() }).where(and(eq(workflows.tenantId, access.tenant.id), eq(workflows.projectId, access.project.id), eq(workflows.id, workflow.id))); redirect(`/t/${access.tenant.slug}/projects/${access.project.slug}/automation/${workflow.slug}`); }
